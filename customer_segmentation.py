...
df.groupby(["Segmentation", "Graduated", "Gender", "Spending_Score"]).agg(
    {"Segmentation": "count"}
)
...
# mode
df_dummy["Ever_Married"] = df_dummy["Ever_Married"].fillna(
    df_dummy["Ever_Married"].mode()[0]
)  # add [0]


df_dummy["Graduated"] = df_dummy["Graduated"].fillna(df_dummy["Graduated"].mode()[0])

df_dummy["Profession"] = df_dummy["Profession"].fillna(df_dummy["Profession"].mode()[0])

df_dummy["Work_Experience"] = df_dummy["Work_Experience"].fillna(
    df_dummy["Work_Experience"].median()
)

df_dummy["Family_Size"] = df_dummy["Family_Size"].fillna(
    df_dummy["Family_Size"].median()
)
...
for con in cont_column:
    if con == "ID":
        ...
        print(
            lr.score(np.expand_dims(df_dummy[con], axis=-1), df_dummy["Segmentation"])
        )
...
hit = model.fit(
    X_train, y_train, batch_size=64, validation_data=(X_test, y_test), epochs=10
)
